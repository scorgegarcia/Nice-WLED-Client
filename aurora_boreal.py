...
from PyQt6.QtWidgets import QLabel, QSlider, QCheckBox, QLineEdit, QHBoxLayout
...
class AuroraBoreal(BaseAnimation):
    ...
    def build_ui(self, layout):
        ...
        def add_color_input(label, key):
            ...
            txt = QLineEdit(str(self.props[key]))
            txt.setPlaceholderText("#00ffaa")

            def update_text(text):
                self.props[key] = text.strip()
                self.save_props()

            txt.textChanged.connect(update_text)
            row.addWidget(txt)
